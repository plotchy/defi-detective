...
async fn main() {
    ...
    let builder = Config::builder().add_source(File::with_name("configuration.yaml"));
    ...
    if clean {
        ...
        let abs_match_output_path = format!(
            "{}/{}",
        ...
        let network_addresses_vec = matches
            .matches
            .iter()
            .map(|match_| (match_.network.clone(), match_.address.clone()))
        ...
        let abs_fetched_addresses_path = format!(
            "{}/{}",
            std::env::current_dir().unwrap().to_str().unwrap(),
            &fetch_settings.rel_db_dir
        );
        for entry in WalkDir::new(abs_fetched_addresses_path)
            .into_iter()
            .filter_map(|e| e.ok())
    ...
    let node_watcher_handle = tokio::spawn(async move {
        run_node_watcher(fetch_settings, node_watcher_tx)
            .await
            .expect("Node watcher failed");
    });

    // run bytecode analyzer
    let bytecode_analyzer_handle = tokio::spawn(async move {
        run_bytecode_analyzer(bytecode_settings, bytecode_analyzer_rx)
            .await
            .expect("Bytecode analyzer failed");
    });

    let endpoint_msg_handle = tokio::spawn(async move {
        run_endpoint_handler()
            .await
            .expect("Endpoint msg handler failed");
    });

    // run threads until termination / errors
    let join_result = tokio::join!(
        node_watcher_handle,
        bytecode_analyzer_handle,
        endpoint_msg_handle
    );
...
pub fn jsons_to_abi(abs_json_dir: &str) {
    ...
    for entry in WalkDir::new(abs_json_dir)
        .into_iter()
        .filter_map(|e| e.ok())
        .filter(|e| e.file_type().is_file())
    {
        ...
        if abi.to_string().contains("Max rate limit reached") {
            println!(
                "Max rate limit reached for {}",
                entry.path().to_str().unwrap()
            );
            continue;
        }
        let (events_vec, fns_vec): (Vec<(String, String)>, Vec<(String, String)>) =
            parse_only_abi_dhvani(abi.clone(), &mut events_set, &mut selectors_set);
    ...
    file.write_all(serde_json::to_string_pretty(&events).unwrap().as_bytes())
        .unwrap();
    let mut file = std::fs::File::create("inputs/selectors.json").unwrap();
    file.write_all(serde_json::to_string_pretty(&selectors).unwrap().as_bytes())
        .unwrap();
    let mut file = std::fs::File::create("inputs/protocol_events_fns.json").unwrap();
    file.write_all(serde_json::to_string_pretty(&protocols).unwrap().as_bytes())
        .unwrap();
}

pub fn parse_only_abi_dhvani(
    abi: Value,
    events_set: &mut HashSet<(String, String)>,
    selectors_set: &mut HashSet<(String, String)>,
    ...
    for item in abi.as_array().unwrap() {
        if item["type"].as_str().unwrap() == "event" {
            ...
            signature = format!("{})", signature);
            // println!("{}", signature);
            let keccak_signature = format!(
                "0x{}",
                hex::encode(keccak256(signature.as_bytes()))[0..8].to_string()
            );
